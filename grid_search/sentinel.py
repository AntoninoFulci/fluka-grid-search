...
def main() -> None:
    if len(sys.argv) < 5:
        sys.exit("Usage: sentinel.py config_path output_dir combo_name job_id1 [job_id2 ...]")
    ...
    if combo not in state.data:
        sys.exit(f"[sentinel] ERROR: combo {combo!r} not found in state. Aborting.")
    ...
    n_total = len(state.data[combo]["runs"])
